...
def LGBM_train_and_test_v2(features, train_cols, target_col, train_feats, test_feats, params):
...
def main():
    ...
    preprocessor = Preprocessor_v2(42, test_essays, test_essays_with_upper, tokenizer=tokenizer, method='test',
                                   save_cols=save_cols)
    ...
    # 计算cols与score之间的相关性矩阵,筛选相关性绝对值小于0.01的特征
    ...
    for i in train_feats.columns:
        ...
        if 'up_time' in i:
            drop_cols_lgb.append(i)
    # lgbm v2 features
    lgb_cols_v2 = [col for col in train_feats.columns if
                   col not in target_col + drop_cols + drop_cols_lgb + drop1 + drop2]  # for LGBM

    OOF_PREDS_v2, TEST_PREDS_v2 = LGBM_train_and_test_v2(lgb_cols_v2, train_cols, target_col, train_feats, test_feats,
                                                         params2)
    print('OOF metric LGBM v2 = {:.5f}'.format(
        metrics.mean_squared_error(train_feats[target_col], OOF_PREDS_v2, squared=False)))
